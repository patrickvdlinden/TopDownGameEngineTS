...
function __autoload($className) {
    if (strpos($className, "MapGenerator\\") === 0) {
        require_once "src/" . basename($className) . ".php";
    }
}

use MapGenerator\TileDescriptor;
use MapGenerator\StaticTileDescriptor;
use MapGenerator\AnimatedTileDescriptor;
use MapGenerator\Tileset;
use MapGenerator\Trigger;
use MapGenerator\TeleportCommand;
use MapGenerator\Map;
use MapGenerator\StaticObjectDescriptor;
...
$tileset
    ->describeTile($VOID)
    ->describeTile($WATER)
    ->describeTile($WATER_A)
    ->describeTile($GRASS)
    ->describeTile($GRASS_TL)
    ->describeTile($GRASS_TR)
    ->describeTile($GRASS_BR)
    ->describeTile($GRASS_BL)
    ->describeTile($DIRT)
    ->describeTile($DIRT_TL)
    ->describeTile($DIRT_TR)
    ->describeTile($DIRT_BL)
    ->describeTile($DIRT_BR)
    ->describeTile($SAND)
    ->describeTile($SAND_TL)
    ->describeTile($SAND_TR)
    ->describeTile($SAND_BR)
    ->describeTile($SAND_BL)
    ->describeTile($WALL)
    ->describeTile($WALL_B)
    ->describeTile($DOOR_T)
    ->describeTile($DOOR);
...
/**
 * Set this tile as 'not passable'.
 *
 * @param TileDescriptor $tileDescriptor
...
function X(TileDescriptor $tileDescriptor) {
    ...
    if ($tileDescriptor instanceof StaticTileDescriptor) {
        ...
        $result = new AnimatedTileDescriptor($tileDescriptor->name, $tileDescriptor->label, false, $tileDescriptor->textureIndice, $tileDescriptor->interval);
    }
...
$map->addChunk(0, 0)
    ->addTiles([
        ...
        $GRASS,   $GRASS,   $SAND,  X($GRASS), $GRASS,   $GRASS,   $GRASS,   $GRASS,   $GRASS,   $GRASS,   $GRASS,   $GRASS,   $GRASS,   $GRASS,   $GRASS,   $DIRT,
